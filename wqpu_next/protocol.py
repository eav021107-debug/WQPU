...
This module intentionally contains no networking and no blockchain consensus.
It defines the data that those layers must sign/verify so independent nodes can
reach the same result from the same state.
"""
...
class SessionDelegation:
    """Wallet-signed permission for one local ephemeral WQPU session key."""
    ...
    def validate(self) -> None:
        ...
        _require_nonempty("session_pubkey", self.session_pubkey)
...
class ProviderRecord:
    """Chain state advertised by one compute provider wallet."""
...
class GlobalPriceState:
    """Consensus-visible single compute price for one epoch."""
...
@dataclass(frozen=True)
class JobManifest:
    """Immutable commitment describing one distributed inference request."""

    job_id: str
    requester_wallet: str
    requester_session_pubkey: str
    model_hash: str
...
@dataclass(frozen=True)
class WorkReceipt:
    """Provider claim for accepted useful work on one immutable job."""
