...
import argparse, asyncio, hashlib, json, secrets, ssl, time
...
class Relay:
    ...
    def snapshot(self):
        ...
        for nid, c in self.controls.items():
            ...
            nodes.append(info)
        if nodes:
            coord = max(nodes, key=lambda x: (int(x.get("ram_mb",0)), int(x.get("threads",0)), x["node_id"]))["node_id"]
        else:
            coord = None
        return {"type":"nodes","coordinator":coord,"nodes":nodes}
    ...
    async def maybe_start_pair(self, sid: str):
        p = self.pairs.get(sid)
        if not p or p.started or not (p.dial_reader and p.accept_reader):
            return
        p.started = True
        p.dial_writer.write(b"WQPU-READY\n")
        p.accept_writer.write(b"WQPU-READY\n")
        await p.dial_writer.drain()
        await p.accept_writer.drain()
        async def run():
            await asyncio.gather(
                self.pipe(p.dial_reader, p.accept_writer),
                self.pipe(p.accept_reader, p.dial_writer),
            )
            p.done.set()
    ...
    async def handle(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        try:
            ...
            if role == "control":
                ctrl = Control(node, dict(hello.get("info") or {}), writer)
                old = self.controls.get(node)
                if old:
                    try: old.writer.close()
                    except Exception: pass
                self.controls[node] = ctrl
                ...
                try:
                    while True:
                        ...
                        if msg.get("type") == "ping":
                            ...
                            ctrl.info.update(msg.get("info") or {})
                            await self.broadcast()
                finally:
            ...
            if role == "dial":
                ...
                if target not in self.controls or node not in self.controls:
                    raise RuntimeError("source or target offline")
                sid = secrets.token_hex(12)
                p = Pair(dial_reader=reader, dial_writer=writer)
                self.pairs[sid] = p
                await self.send(target, {"type":"open","stream":sid,"service":service})
                try:
                    await asyncio.wait_for(p.done.wait(), timeout=3600)
            ...
            if role == "accept":
                ...
                p.accept_reader, p.accept_writer = reader, writer
...
async def main_async(args):
    ...
    relay = Relay(secret)
    server = await asyncio.start_server(relay.handle, args.host, args.port, ssl=ctx)
    addrs=", ".join(str(s.getsockname()) for s in server.sockets or [])
    print(f"WQPU relay listening on {addrs}", flush=True)
    async with server:
