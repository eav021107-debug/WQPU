...
// providerRecord mirrors chain/x/wqpu/precompile.EncodeProvider. Keeping this
// fixture in native wire order makes a codec drift fail client CI immediately.
...
func TestDecodeProviderRecordMatchesNativeWireFormat(t *testing.T) {
	peerID := testPeer()
	provider, err := DecodeProviderRecord(providerRecord(peerID))
	if err != nil { t.Fatal(err) }
	if provider.PeerID != peerID || provider.ProtocolVersion != 1 || provider.CapacityUnits != 100 || provider.ReportedBusyUnits != 10 || provider.FreeMemoryBytes != 8*1024*1024*1024 || provider.HeartbeatHeight != 100 || provider.ExpiresHeight != 200 {
		t.Fatalf("decoded provider=%+v", provider)
	}
	if len(provider.Endpoints) != 1 || provider.Endpoints[0] != "wqpu://127.0.0.1:7443" || len(provider.ModelHashes) != 1 {
		t.Fatalf("decoded provider lists=%+v", provider)
	}
}
...
func TestResolvePeerRequiresActiveChainRecordAndControlSession(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if peer.Provider.PeerID != peerID || peer.ControlSession != control || len(peer.Provider.Endpoints) != 1 { t.Fatalf("resolved peer=%+v", peer) }
}
...
func TestDecodeProviderRecordRejectsTrailingAndMalformedEndpoint(t *testing.T) {
	...
	endpointStart := 1 + 20 + 32 + 4 + 8 + 8 + 8 + 8 + 8 + 32 + 1 + 2
