...
func TestConnectorPrepareAndCompleteFlow(t *testing.T) {
	...
	defer cancel()
	cfg := connectorConfig()
	connector, err := StartConnector(ctx, cfg)
	if err != nil { t.Fatal(err) }
	...
	var typed TypedData
	if err := json.NewDecoder(prepared.Body).Decode(&typed); err != nil { t.Fatal(err) }
	...
	if typed.Message["sessionAddress"] != cfg.SessionAddress {
		t.Fatalf("wrong session address: %+v", typed.Message)
	}
	...
	if completed.StatusCode != http.StatusOK { t.Fatalf("complete status=%d", completed.StatusCode) }

	select {
	case result := <-connector.Results:
		if result.Wallet != wallet || result.Signature != signature { t.Fatalf("result=%+v", result) }
...
func TestConnectorRejectsWalletSwapAfterPrepare(t *testing.T) {
	...
	_ = resp.Body.Close()
	if resp.StatusCode != http.StatusOK { t.Fatalf("prepare status=%d", resp.StatusCode) }
	resp = postJSON(t, connector.URL+"complete", origin, ConnectResult{
		Wallet: walletB, Signature: "0x" + strings.Repeat("33", 65),
	})
	_ = resp.Body.Close()
	if resp.StatusCode != http.StatusConflict { t.Fatalf("wallet swap status=%d", resp.StatusCode) }
}
