...
find_python() {
  for p in python3.13 python3.12 python3.11 python3.10 python3; do
    if need "$p" && "$p" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)' >/dev/null 2>&1; then
      command -v "$p"
      return 0
    fi
  done
...
if [ -z "$PYTHON" ]; then
  echo "WQPU: installing a compatible Python..."
  if need apt-get; then
    $SUDO apt-get update
    for pkg in python3.13 python3.12 python3.11 python3.10 python3; do
...
if [ -z "$PYTHON" ]; then
  echo "WQPU could not install Python 3.10+ automatically on this system." >&2
  exit 1
fi
