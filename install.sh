...
CACHE_BUSTER="meter-0.6.0-dev-r1"
...
for file in wqpu.py wqpu_chain.py wqpu_wallet.py wqpu_session.py wqpu_meter.py wqpu_payments.py wqpu_runtime.py network-config.json; do
  curl -fsSL --retry 3 "${RAW}/${file}?installer=${CACHE_BUSTER}" -o "$ROOT/$file"
done
chmod 755 "$ROOT/wqpu.py" "$ROOT/wqpu_chain.py" "$ROOT/wqpu_wallet.py" "$ROOT/wqpu_session.py" "$ROOT/wqpu_meter.py" "$ROOT/wqpu_payments.py" "$ROOT/wqpu_runtime.py"
