...
func TestReceiptMustAdvanceMonotonically(t *testing.T) {
	...
	if err := ValidateReceipt(job, nil, first); err != nil {
		t.Fatal(err)
	}
	second := receipt("peer-a", "wallet-a", 2, 50, 150)
	if err := ValidateReceipt(job, &first, second); err != nil {
		t.Fatal(err)
	}
	if err := ValidateReceipt(job, &second, second); err == nil {
...
func TestReceiptCannotChooseItsOwnPrice(t *testing.T) {
	...
	if err := ValidateReceipt(job, nil, r); err == nil {
		t.Fatal("receipt with provider-chosen price should be rejected")
	}
}
...
func TestReceiptCannotClaimUnreservedProvider(t *testing.T) {
	...
	if err := ValidateReceipt(job, nil, r); err == nil {
		t.Fatal("unreserved provider should be rejected")
	}
}
...
func TestSettlementPaysEveryReservedProviderAtGlobalPrice(t *testing.T) {
	...
	if settlement.TotalCharge != 350 {
		t.Fatalf("total charge=%d", settlement.TotalCharge)
	}
	if settlement.Payouts["wallet-a"] != 150 || settlement.Payouts["wallet-b"] != 200 {
		t.Fatalf("payouts=%+v", settlement.Payouts)
	}
}

func TestSettlementRequiresAllReservedProviders(t *testing.T) {
	job := receiptJob()
	_, err := BuildSettlement(job, map[string]WorkReceipt{
		"peer-a": receipt("peer-a", "wallet-a", 1, 100, 100),
	})
	if err == nil {
...
func TestSettlementCannotExceedTotalJobCompute(t *testing.T) {
	...
	if _, err := BuildSettlement(job, latest); err == nil {
		t.Fatal("aggregate compute above job maximum should fail")
	}
}
